...
ss_op_selector()
{
  ...
  if constexpr (is_same_v<ElementA, bf16> || is_same_v<ElementA, fp16>) {
    ...
    if constexpr (opType == OpType::Cluster) {
      return XE4_ASYNC_GMMA_MULTICAST<ElementD, ElementC, ElementB, ElementA, MMA_Shape, isRowMajorA, isRowMajorB, MatrixDesc, Abarrier>();
    } else {
      return XE4_ASYNC_GMMA<ElementD, ElementC, ElementB, ElementA, MMA_Shape, isRowMajorA, isRowMajorB, MatrixDesc, Abarrier>();
    }
  }
